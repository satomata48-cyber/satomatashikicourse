			...
			const { data: space, error: createError } = await supabase
				...
				.insert({
					...
					landing_page_content: {
						sections: [
							{
								...
								content: {
									title: formData.title,
									subtitle: formData.description || '素晴らしい学習体験をお届けします',
							...
							},
							{
								...
								content: {
									...
									features: [
										{
											title: '実践的なカリキュラム',
											description: '現場で使える知識とスキルを身に付けます',
											icon: 'academic-cap'
										},
										{
											title: '個別サポート',
											description: '一人ひとりの学習進度に合わせたサポート',
											icon: 'user-group'
										},
										{
											title: '終了後のフォロー',
											description: 'コース修了後も継続的なサポートを提供',
											icon: 'shield-check'
										}
									]
								}
						...
						metadata: {
							title: formData.title,
							description: formData.description || '最高の学習体験をお届けします',
							seoTitle: `${formData.title} | オンライン学習`,
						...
						navigation: {
							showStudentAuth: true,
							studentLoginUrl: `/${username}/space/${formData.slug}/login`,
							studentDashboardUrl: `/${username}/space/${formData.slug}/dashboard`,
							topNavigation: {
								enabled: true,
								showSpaceTitle: true,
								showLogo: true,
								logoText: formData.title.charAt(0),
								style: 'clean' // トップページ風のクリーンなデザイン
							}
						}
					}
				})
