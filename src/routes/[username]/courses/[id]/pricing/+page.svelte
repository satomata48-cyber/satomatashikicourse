<script lang="ts">
	...
	async function setupStripeProduct() {
		...
		try {
			...
			const { productId, priceId, paymentLink, testMode } = await response.json()
			...
			const modeText = testMode ? 'テストモード' : '本番モード'
			successMessage = `Stripe決済が設定されました！（${modeText}）`
								...
								<div class="flex items-center justify-between">
									<div class="flex items-center">
										...
										<span class="text-sm font-medium" class:text-blue-800={isTestMode} class:text-green-800={!isTestMode}>
											{isTestMode ? '現在テストモードです' : '現在本番モードです'}
										</span>
									</div>
									<a
										href="/{username}/payment-settings"
										class="text-xs font-medium underline" class:text-blue-700={isTestMode} class:text-green-700={!isTestMode}
									>
										モード切り替え
									</a>
								</div>
